...
class PriorityQueue {
    static final int CAPACITY = 10; // FIXED CAPACITY OF THE QUEUE the capacity cannot change there can only be 10 tasks in the queue.
    Job[] queue;                    // the queue variable is an array of Job objects defined by the client or user
    int size = 0;                   // the size variable is an integer that will keep track of the number of tasks in the queue.
    int front = 0;                  // the front variable is an integer that will keep track of the front of the queue.
    int rear = 0;                   // the rear variable is an integer that will keep track of the rear of the queue.
    public PriorityQueue() {
        queue = new Job[CAPACITY];          // Initialize the queue array that will serve as a queue ADT with the fixed capacity defined by the professor to be 10.
        size = 0;                           // Initialize the size of the queue to 0. 
    }

    public boolean isEmpty() {
        return size == 0;                   // return true if the size of the queue's size is currently 0(empty), otherwise return false.
    }
    ...
    public void clear() {
        size = 0;                           // set the size of the queue to 0 to clear the queue                                                (this forces the queue to overwrite itself**?).
    }
/*
 * THIS NEXT SECTION MAY NEED TO BE CHANGED IT DEPENDS ON CIRCULAR QUEUE IMPLEMENTATION IM NOT SURE RIGHT NOW
 */
    ...
    public String dequeue() {
        ...
        Job removedJob = queue[front];
        queue[front] = null; // Clear the dequeued job
    ...
    // return the priority of the task with the highest priority without removing it from the queue.
    // User MUST check if queue is empty before calling this method.
    // This method will print the task with the highest priority without removing it from the queue AND will not disclose the actual priority of the task.
    // So long as upheap/downheap are implemented correctly, this method will work as intended otherwise without a correctly implemented priority queue this method will not work as intended.
    public int peekPriority() {
        return queue[front].getPriority(); // return the priority of the task with the highest priority without removing it from the queue. 
    }
    public String peekTask() {
        return queue[front].getTaskName(); // return the task with the highest priority without removing it from the queue.
    }
    ...
    void downheap(int k) { 
        ...
        while (k <= N / 2) {                        // while k has at least one child
            ...
            if (j < N && a[j] < a[j + 1]) j++;      // j becomes index of larger child

            if (v >= a[j]) break;                   // if current value is in correct position, stop
...
public class PqTest {
    ...
    // This method runs the user's choice based on the menu options. It serves as a more readable alternative to a switch statement.
    // It checks the user's choice and calls the appropriate method to perform the action. 
    
    static void runChoice(int choice) {
        if (choice == 0) exitProgram();         // Exit the program.
        if (choice == 1) enqueueTask();         // Enqueue a task.
        if (choice == 2) dequeueTask();         // Dequeue a task.
        if (choice == 3) System.out.println("most urgent task: "+ pq.peekTask());                     // Peek at the most urgent task.
        if (choice == 4) System.out.println("priority of most urgent task: "+ pq.peekPriority());     // Peek at the priority of the most urgent task.
        if (choice == 5) clearQueue();          // Clear the queue.
        if (choice == 6) printSize();           // Get queue size.
        if (choice == 7) printIsEmpty();        // Check if queue is empty.
    }
    ...
    static void dequeueTask() {
        if (!pq.isEmpty()) System.out.println("Removed: " + pq.dequeue());
...
class Job {
    ...
    public Job(String taskName, int priority) {
